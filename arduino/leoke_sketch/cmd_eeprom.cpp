...
#define LEOKE_EEPROM_DATASTART 5
...
static unsigned short crc16(unsigned char newchar, unsigned short crcval) {
	return (crcval >> 8) ^ crc16_table[(crcval ^ newchar) & 0x00ff];
}

void cmd_eeprom(Stream * ser, int argc, char ** argv) {
	if (argc == 1 && !strcmp(argv[0], "load")) {
		unsigned int i;
		unsigned short crc = 0xFFFF, crc_stored;
		char * pdata = (char *) &pads;
		if (!(EEPROM.read(0) == 'L' &&
		      EEPROM.read(1) == 'E' &&
		      EEPROM.read(2) == 'O' &&
		      EEPROM.read(3) == 'K') && ser) {
		...
		EEPROM.write(3, 'K');
		for (i = 0; i < sizeof(pads); ++i) {
			EEPROM.write(LEOKE_EEPROM_DATASTART + i, *pdata);
			crc = crc16(*pdata, crc);
			++pdata;
		}
